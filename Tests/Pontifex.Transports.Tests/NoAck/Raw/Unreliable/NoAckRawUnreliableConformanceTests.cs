...
public abstract class NoAckRawUnreliableConformanceTests
{
    private static readonly TimeSpan DeliveryTimeout = TimeSpan.FromSeconds(2);

    protected abstract INoAckRawUnreliableConformanceAdapter CreateAdapter();
    ...
    public async Task AcceptedSend_DoesNotBeginReceiveCallbackBeforeTrySendReturns()
    {
        ...
        var clientControl = GetControl<INoAckRawUnreliableConformanceControl>(client);
    ...
    public async Task Stop_PreventsBlockedReceiveCallbackFromBeginning()
    {
        ...
        var control = GetControl<INoAckRawUnreliableConformanceControl>(fixture.Server);
    ...
    private static void EnableReliable(INoAckRawUnreliableClient client)
    {
        var control = GetControl<INoAckRawUnreliableClientConformanceControl>(client);
        if (!control.TryMakeReliable())
            Assert.Ignore("The implementation does not provide reliable debug mode.");
    }

    private static void Start(INoAckRawUnreliableServer server, INoAckRawUnreliableClient client)
    {
        Assert.That(server.Start(_ => { }), Is.True);
        StartClient(client);
    }

    private static void StartClient(INoAckRawUnreliableClient client)
    {
        Assert.That(client.Start(_ => { }), Is.True);
    }
    ...
    private static UnionDataList CreateComplexMessage(ITransport transport)
    {
        ...
        message.PutFirst("NoAckRawUnreliable");
        return message;
    }
    ...
    {
        var message = CreateEmptyMessage(transport);
        var bytes = transport.Memory.ByteArraysPool.Acquire(transport is INoAckRawUnreliableClient client
            ? client.MessageMaxByteSize
            : ((INoAckRawUnreliableServer)transport).MessageMaxByteSize);
...
[TestFixture]
public sealed class DirectNoAckRawUnreliableConformanceTests : NoAckRawUnreliableConformanceTests
{
    protected override INoAckRawUnreliableConformanceAdapter CreateAdapter()
    {
        return new DirectNoAckRawUnreliableConformanceAdapter();
    }
}
