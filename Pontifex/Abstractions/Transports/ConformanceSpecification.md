# Transport Carrier-Independent Core Conformance Profile
...
transport contract that is independent of any specific carrier or protocol.
It is intended for a conformance suite that must run against implementations
using their ordinary, uncontrolled physical carrier.
...
certify the complete transport specification. It certifies only local API
behavior whose outcome does not depend on a carrier delivering a packet.

A passing result is named **Carrier-Independent Core Conformant**. It is not a
claim of full transport conformance.

## 2. Relationship to the transport specification

The transport-specific specification remains the normative contract.
This profile neither weakens nor replaces it.
...
The callback is controlled by the `BeforeStoppedCallbackGate` property. Repeated
`Stop` calls while that gate is paused **MUST NOT** cause duplicate callback
dispatch.

### 5.5 Status properties

`IsValid`, `IsStarted`, and any transport-specific maximum-message-size property
**MUST** be safe for concurrent reads while other threads start, stop, or force
failure. Reads **MUST NOT** throw or deadlock.

Transport-specific maximum-message-size properties **MUST** remain immutable for
...
A test suite that communicates directly through an uncontrolled physical carrier
...
1. Implement the transport-specific companion specification in full.
