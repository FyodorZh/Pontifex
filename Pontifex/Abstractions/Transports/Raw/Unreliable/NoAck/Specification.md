...
| **transport** | One `IRawUnreliableNoAckClient` or `IRawUnreliableNoAckServer` instance. |
...
| **source route** | A server-side route identified by an `IEndPoint` supplied to the server handler factory from an inbound message source. |
...
| Conformance | An implementation claiming Carrier-Independent Core Conformance exposes the specified transport and endpoint test controls. | Use controls only in conformance adapters and never as application-plane behavior. | Whether an ordinary production instance exposes no controls or controls that remain inactive. |
...
`Func<IEndPoint, IRawUnreliableHandler?>`.
...
route, the server **MUST** invoke the configured factory with that source
`IEndPoint`. Route creation is atomic for equal sources: concurrent first
messages from one equal source **MUST NOT** cause multiple concurrent factory
calls or multiple endpoints. If the factory returns a handler, the server
creates one endpoint for the route, whose `RemoteEndPoint` compares equal to
the `IEndPoint` supplied to the factory. The server then invokes
...
sequenceDiagram
    ...
    ST->>SF: handlerFactory(source), if route is new
...
RawUnreliableNoAck **MUST NOT** return `NotConnected`.
...
solely a conformance aid and does not change RawUnreliableNoAck production
semantics.
...
This revision is a breaking raw-unreliable contract transition. It removes the
RawUnreliableNoAck transport receive events and transport-level `TrySend`
methods in favor of endpoint-owned `UnreliableSend`. It also changes the shared
`IRawUnreliableHandler` contract by adding `OnStopped` and changes
`IRawUnreliableEndpoint.Stop` to return `bool` and accept an optional reason.
...
controls defined above. Any component that inherits from, exposes, adapts, or
otherwise consumes these shared raw-unreliable abstractions MUST be updated to
...
- [ ] Expose the split transport and endpoint conformance controls from
