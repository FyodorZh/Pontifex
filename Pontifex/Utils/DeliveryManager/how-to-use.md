...
dm.Received += (id, data) =>
{
    Console.WriteLine($"Received {id}: {data.Elements.Count} elements");
...
Messages up to `maxPacketSize - 6 - SafetyMargin(4)` bytes go as a single wire packet.
...
Messages up to `(maxPacketSize - 10 - SafetyMargin(4)) × 255` bytes are supported.
...
| Type | Byte 0 | Payload |
|---|---|---|
| UserSingle | 0x00 | `[DeliveryId:2][userData:N]` |
| UserMulti | 0x01 | `[DeliveryId:2][PartId:1][PartsNum:1][chunkData:N]` |
| DeliveryInfo | 0x02 | `[Count:2][{DeliveryId:2}{ChunkId:1} × Count]` |

`DeliveryInfo` is always generated by the DeliveryManager automatically — you never create it manually.
