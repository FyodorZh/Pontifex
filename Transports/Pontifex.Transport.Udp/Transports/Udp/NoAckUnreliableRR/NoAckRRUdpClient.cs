...
namespace Pontifex.Transports.Udp
{
    internal sealed class NoAckRRUdpClient : AbstractTransport, INoAckUnreliableRRClient, INoAckUnreliableRRServerEndpoint
    {
        ...
        public NoAckRRUdpClient(IPAddress ipAddress, int port)
        {
        ...
        SendResult INoAckUnreliableRRServerEndpoint.Send(IMacroOwner<Message> messages)
        {
            if (messages == null)
            {
        ...
        private void OnReceived(EndPoint remoteEp, IMacroOwner<Message> messages)
        {
            ...
            if (handler != null)
            {
                foreach (var message in messages.Enumerate())
                {
                    try
                    {
                        handler.Received(message.Acquire());
                    }
                    catch (Exception e)
                    {
